...
async function main() {
  ...
  // Save old controller address
  const oldController = deployments.contracts.ETCRegistrarController

  // 1. Deploy new ETCRegistrarController with reserved names support
  console.log('\n1. Deploying ETCRegistrarController v2 (with reserved names)...')
  ...
  console.log('  ETCRegistrarController v2 deployed at:', newController)
  ...
  // 3. Deploy new PublicResolver with new controller
  console.log('\n3. Deploying PublicResolver with new controller...')
  ...
  // 4. Update default resolver on ReverseRegistrar
  ...
  deployments.contracts.ETCRegistrarControllerV1 = oldController // Keep old for reference
  ...
  console.log('\n=== Deployment Complete ===')
  console.log('ETCRegistrarController v2:', newController)
  console.log('PublicResolver:', newResolver)
  console.log('\nNew features:')
  console.log('  - reserveName(label) - Reserve a single name')
  console.log('  - reserveNames(labels[]) - Reserve multiple names')
  console.log('  - unreserveName(label) - Unreserve a name')
  console.log('  - isReserved(label) - Check if a name is reserved')
  console.log('\nNext step: Run reserve-names.ts to initialize reserved names')
}
